...
public class TreeFake<T> {
    Node<T> root;

    public TreeFake(){}

    public TreeFake(T root){
    ...
    public Node<T> rebalanceMain(Node<T> node, int difference){
        ...
        if (difference>0){
            //Node<T> temp = node;
            if (parent.compareTo(node.getValue())>0){//izquierda
                node = rotateRight(node);
                parent.setLeftSon(node);
                System.out.println();
                return node;
            }
            if (parent.compareTo(node.getValue())<0){
                node = rotateLeft(node.getLeftSon());
        ...
        if (difference<0){
            ...
            if (parent.compareTo(node.getValue())<0){
                node = rotateRight(node.getRightSon());
                node.setRightSon(node);
                node = rotateLeft(node);
                parent.setRightSon(node);
                return node;
            }
    ...
    public Node<T> rebalance(Node<T> node){
        if (node != null){
            ...
            if (Math.abs(left-right)>1){
                node = rebalanceMain(node, left-right);
                return  node;
            }
        }
        return node;
